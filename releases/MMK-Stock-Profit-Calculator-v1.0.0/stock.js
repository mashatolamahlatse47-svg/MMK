document.addEventListener("DOMContentLoaded", () => {
  ...
  function loadRecords() {
    if (window.MMKOffline) {
      const data = MMKOffline.load();
  ...
  function sellProduct(id) {
    ...
    if (window.MMKOffline) {
      ...
      const revenue = Number(product.price);
      const profit = revenue - Number(product.cost);
  ...
  function renderSalesHistory() {
    ...
    const revenueElement = document.getElementById("salesRevenue");
    const profitElement = document.getElementById("salesProfit");
    const tableBody = document.getElementById("salesTableBody");

    if (!countElement || !revenueElement || !profitElement || !tableBody) {
      return;
    }
    ...
    revenueElement.textContent = "R" + totalRevenue.toFixed(2);
    profitElement.textContent = "R" + totalProfit.toFixed(2);
    ...
    sales.slice().reverse().forEach((sale) => {
      ...
      row.innerHTML = `
        ...
        <td style="padding:8px; text-align:right;">R${Number(sale.revenue).toFixed(2)}</td>
        <td style="padding:8px; text-align:right;">R${Number(sale.profit).toFixed(2)}</td>
      `;
